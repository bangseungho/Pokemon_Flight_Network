...
void SceneManager::LoadScene(const HWND& hWnd)
{
	...
	if (GET_SINGLE(Network)->IsConnected()) {
		uint8 curPhase{};
		if ((uint8)crntScene >= (uint8)Scene::Stage)
			curPhase = phase.GetPhase();

		SceneData sendData{ GET_SINGLE(Network)->GetClientIndex(), 0, static_cast<uint8>(crntScene), gameData.ClearRecord, stage.GetAirPokemon(), stage.GetLandPokemon(), curPhase };
