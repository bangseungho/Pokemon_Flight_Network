...
SkillManager::SkillManager(shared_ptr<Player> player)
{
	this->mPlayer = player;
	...
	{
	case Type::Elec:
		imgSkill_Elec_Q.Load(_T("..\\Client\\images\\battle\\sprite_skill_elec.png"), { 34,226 }, 10, 0xdf);
		skillEffect = new Effect(imgSkill_Elec_Q, type);
		break;
	case Type::Fire:
		imgSkill_Fire_Q.Load(_T("..\\Client\\images\\battle\\sprite_skill_fire.png"), { 80,96 }, 50);
		skillEffect = new Effect(imgSkill_Fire_Q, type);
		break;
	case Type::Water:
		imgSkill_Water_Q.Load(_T("..\\Client\\images\\battle\\skill_water.png"), { 273,843 }, 50, 0xaf);
...
bool BossSkillManager::Effect::Animate()
{
	...
	else if (skillData.damage > 0 && skillData.isActiveDamage == true)
	{
		...
		if (skillData.isRotated == true)
		{
			...
			{
				//mPlayer->Hit(skillData.damage, mBoss->GetType());
...
{
	switch (mBoss->GetType())
	{
	case Type::Elec:
		imgSkill1.Load(_T("..\\Client\\images\\battle\\sprite_boss_skill1_elec.png"), { 32,224 }, 9);
		imgSkill1.ScaleImage(1.0f, 6.0f);
		imgSkill1_Warning.Load(_T("..\\Client\\images\\battle\\boss_skill1_elec_warning.png"), { 31,223 }, 16, 0x10);
		imgSkill1_Warning.ScaleImage(1.0f, 6.0f);
		imgSkill2.Load(_T("..\\Client\\images\\battle\\sprite_boss_skill2_elec.png"), { 80,80 }, 15);
		imgSkill2.ScaleImage(0.5f, 0.5f);
		imgSkill2_Warning.Load(_T("..\\Client\\images\\battle\\boss_warning_circle.png"), { 79,79 }, 15, 0x00);
		imgSkill2_Warning.ScaleImage(0.5f, 0.5f);
		break;
	case Type::Water:
		imgSkill1.Load(_T("..\\Client\\images\\battle\\boss_skill1_water.png"), { 273,712 });
		imgSkill1.ScaleImage(0.25f, 0.25f);
		imgSkill2.Load(_T("..\\Client\\images\\battle\\sprite_boss_skill2_water.png"), { 56,56 }, 14);
		imgSkill2.ScaleImage(0.5f, 0.5f);
		imgSkill2_Warning.Load(_T("..\\Client\\images\\battle\\boss_warning_circle.png"), { 79,79 }, 15, 0x00);
		imgSkill2_Warning.ScaleImage(0.5f, 0.5f);
		break;
	case Type::Fire:
		imgSkill1.Load(_T("..\\Client\\images\\battle\\boss_skill1_fire.png"), { 17,30 });
		imgSkill1.ScaleImage(2.0f, 2.0f);
		imgSkill2.Load(_T("..\\Client\\images\\battle\\boss_skill2_fire.png"), { 173,230 });
		imgSkill2.ScaleImage(1.5f, 1.5f);
		imgSkill2_Warning.Load(_T("..\\Client\\images\\battle\\boss_skill2_fire_warning.png"), { 11,223 }, 8, 0x00);
		imgSkill2_Warning.ScaleImage(1.0f, 6.0f);
		break;
	case Type::Dark:
		posOrigins = new Vector2[dark_Skill1Count];
		imgSkill1.Load(_T("..\\Client\\images\\battle\\bullet_boss_dark.png"), { 699,699 });
		imgSkill1.ScaleImage(0.05f, 0.05f);
		imgSkill2.Load(_T("..\\Client\\images\\battle\\boss_skill2_dark.png"), { 11,33 });
		imgSkill2.ScaleImage(1.5f, 1.5f);
		imgSkill2_Warning.Load(_T("..\\Client\\images\\battle\\boss_skill2_dark_warning.png"), { 100,100 }, 200, 0x50);
...
void BossSkillManager::UseSkill()
{
	...
	{
	case Type::Elec:
		if (mBoss->GetAct() == BossAct::Skill1)
		{
			Skill1_Elec_Create();
		}
		else if (mBoss->GetAct() == BossAct::Skill2)
		{
			Skill2_Elec_Create();
		}
		break;
...
void BossSkillManager::Skill1_Elec_Create()
{
	...
	{
		skillEffects.emplace_back(imgSkill1_Warning, mBoss->GetPosCenter(), startDegree + (20.0f * i));
	}
}
void BossSkillManager::Skill1_Elec()
{
	...
	if (isWarningEnd == true)
	{
		...
		SkillData skillData;
		skillData.damage = mBoss->GetDamage_Skill1();
		for (const Vector2& unitVector : unitVectors)
		{
			skillEffects.emplace_back(imgSkill1, mBoss->GetPosCenter(), unitVector, skillData);
		}
	}
...
void BossSkillManager::Skill2_Elec()
{
	...
	SkillData skillData;
	skillData.damage = mBoss->GetDamage_Skill2();
...
void BossSkillManager::Skill1_Fire()
{
	...
	if (creationCount < creationMaxCount && crntSkillDelay <= 0)
	{
		...
		const FRECT rectBoss = mBoss->GetRectBody();
		const int width = mBoss->GetBodyWidth();
		const int height = mBoss->GetBodyHeight();
...
void BossSkillManager::Skill1_Dark()
{
	...
	if (creationCount < creationMaxCount && crntSkillDelay <= 0)
	{
		...
		const FRECT rectBoss(mBoss->GetRectBody());
		const float mBossHeight = mBoss->GetBodyHeight();

		Vector2 posCenter = { 0, };
		posCenter.x = rectDisplay.left + (rand() % displayWidth);
		posCenter.y = rectBoss.bottom - mBossHeight / 2;
	...
	for (int i = 0; i < creationCount; ++i)
	{
		if (posOrigins[i] != Vector2::Zero())
		{
			...
			if (posOrigins[i].y > (rectDisplay.bottom + (darkSkilldata.radius * 2.5f)))
			{
